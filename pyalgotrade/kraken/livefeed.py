...
from pyalgotrade.kraken.netclients import BookPoller



class LiveBookFeed(barfeed.BaseBarFeed):
    """A streaming book feed
    """

    QUEUE_TIMEOUT = 0.01

    def __init__(self, httpClient, maxLen=None):
        super(LiveTradeFeed, self).__init__(bar.Frequency.SECOND, maxLen)
        self.__poller = BookPoller(httpClient, 1)
    ...
    def __dispatchImpl(self, eventFilter):
        try:
            eventType, eventData = self.__poller.get(True, LiveBookFeed.QUEUE_TIMEOUT)
        except Queue.Empty:
            return False
    ...
    def dispatch(self):
        ...
        if super(BookFeed, self).dispatch(): ret = True
        return ret
